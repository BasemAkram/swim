...
if st.button("🚀 تشغيل التحليل العام", use_container_width=True, type="primary"):
    ...
    # دالة التنبؤ باستخدام المودل الخاص بالمسافة
    def predict_time(row, fatigue_default):
        ...
        # b دائمًا يأخذ قيمة الـ slider
        b = fatigue_default

        # اختيار المودل بناءً على d2
        model = st.session_state.models.get(int(d2), None)

        if model:
            log_d2 = np.log(d2)
            log_t_pred = model.predict([[log_d2]])[0]
            t_pred = np.exp(log_t_pred)
        else:
            available_models = [dist for dist, m in st.session_state.models.items() if m is not None]
            nearest_d = min([dist for dist in available_models if dist >= d2], default=None)
            nearest_model = st.session_state.models.get(nearest_d, None) if nearest_d else None

            if nearest_model:
                log_t_pred_nearest = nearest_model.predict([[np.log(nearest_d)]])[0]
                t_pred = np.exp(log_t_pred_nearest) * (d2 / nearest_d) ** b
            elif len(df) >= 2:
                distances = df["المسافة الحالية (م)"].values
                times = df["الزمن الحالي (ث)"].values
                log_d = np.log(distances)
                log_t = np.log(times)
                temp_model = LinearRegression()
                temp_model.fit(log_d.reshape(-1, 1), log_t)
                log_d2 = np.log(d2)
                log_t_pred = temp_model.predict([[log_d2]])[0]
                t_pred = np.exp(log_t_pred) * (d2 / distances.mean()) ** b
            else:
                t_pred = t1 * (d2 / d1) ** b if d1 > 0 else 0
        ...
        # السرعة المتوقعة بالمعادلة للـ 100%
    ...
    for idx, row in df.iterrows():
        ...
        new_row["زمن 100% (ث)"] = round(t_100, 2)
    ...
    res_df["درجة السرعة"] = (
            res_df["السرعة الحالية (م/ث)"] /
            res_df["السرعة الحالية (م/ث)"].max() * 100
    ).round(1).fillna(0)
    ...
    res_df["الكفاءة الفنية الواقعية"] = (
            res_df["درجة السرعة"] * 0.60 +
            res_df["درجة التحمل"] * 0.40
    ).round(1).fillna(0)
...
if st.session_state.results is not None:
    ...
    # عرض متوسطات عامة
    team_avg_speed = res["السرعة الحالية (م/ث)"].mean()
    team_avg_eff = res["الكفاءة الفنية الواقعية"].mean()
    team_avg_drop = res["نسبة هبوط السرعة (%)"].mean()
    ...
    fig_scatter.add_trace(go.Scatter(
        x=res["نسبة هبوط السرعة (%)"],
        y=res["السرعة الحالية (م/ث)"],
        mode='markers',
    ...
    with t_report:
        ...
        report_cols = [
                          "ID",
                          "الاسم",  # 👈 أضف هذا السطر هنا ليظهر في الجدول والتحميل
                          "المسافة الحالية (م)",
                          "الزمن الحالي (ث)",
                          "المسافة المستهدفة (م)"
                      ] + [f"زمن متوقع {e} (ث)" for e in effort_list if f"زمن متوقع {e} (ث)" in df.columns] + [
                          "السرعة الحالية (م/ث)", "السرعة المتوقعة (م/ث)",
                          "الكفاءة الفنية الواقعية", "نسبة هبوط السرعة (%)", "درجة التحمل",
